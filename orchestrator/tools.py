"""Agent tool layer: schema budgeting, the tool-guard gate, execution, and result shaping.

Split out of the agent god-module (part of the in-progress prism_core). All provider access
is via module attributes (search.search / toolserver.post / fireworks.complete) so the test
monkeypatch seam at those source modules keeps working. No agent-runtime dependencies.
"""
import json
import re

from . import config, fireworks, search, toolserver
from .owui import _last_user_text
from .prompts import SYSTEM_TOOL_GUARD, TOOL_SCHEMAS
