...
SYSTEM_VISION = (
    ...
    "## EVIDENCE TRANSCRIPT\n"
    "A faithful, LITERAL extraction of what is actually on the image — no interpretation, no "
    "answer, and INDEPENDENT of the specific question (it is reused as the durable record of "
    "this image, so transcribe the image itself, not just the part this question asks about). "
    "SCALE the detail to the CONTENT TYPE: for a DOCUMENT, table, chart, form, or text-bearing "
    "image (a paper page, a data table, a spreadsheet, a receipt, a figure) be EXHAUSTIVE — quote "
    "text verbatim, render every table as a Markdown table with exact cell values, and for a "
    "chart give the title, axes, series, and read-off values. For a general photo or a busy UI / "
    "desktop screenshot, give a thorough STRUCTURAL overview of everything salient that's actually "
    "there (subjects, layout, the main visible text/labels) — but do NOT exhaustively transcribe "
    "every incidental filename, menu item, or decorative pixel, and never guess. Label distinct "
    "regions [T1], [T2], … for citation. If a value is unclear, write [unclear].\n\n"
    "## READING\n"
    "Now address the user's actual request about the image — this part IS question-specific. "
    "State your answer, including reasonable visual IDENTIFICATIONS (a breed, a landmark, an "
    "object/model) when the image clearly supports them. After each claim, cite the transcript "
    "region(s) it rests on, e.g. '(from [T2])'. Do not assert facts the image doesn't show; if it "
